...
export default defineConfig({
  ...
  // One retry everywhere: locally a cold `next dev` compiles routes on first
  // visit, which can be slow enough to trip an assertion on a busy machine; the
  // retry runs with warm routes. CI uses a prebuilt server, so retries there
  // only guard genuine flakiness.
  ...
  use: {
    ...
    // Generous enough to absorb on-demand dev compiles of a route's first visit.
